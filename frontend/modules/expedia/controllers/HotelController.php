...
class HotelController extends RestController {
    ...
    public function actionGetList()  {
        ...
        try {
                ...
                $curl = new curl\Curl();
                $data = $curl->get($this->ean_base_url . 'list?'.http_build_query($final_request));
    ...
    public function actionGetInfo($id){
        $device = $this->checkAuth();
        try {
            $customer_session = isset($_GET['customerSessionId'])?$_GET['customerSessionId']:false;
            if($customer_session){
                $get = array('hotelid'=>$id,'customerSessionId'=>$customer_session);
            }else{
                $get = array('hotelid'=>$id);
            }
            //$postdata = JSON::decode(file_get_contents("php://input"));
            //$roominfo = array('single_room' => $postdata['single_room'], 'double_room' => $postdata['double_room']);
            $config = $this->expediaConfig();
            $final_request = array_merge($config,$get);
            Logger::add($final_request,'request');
            $curl = new curl\Curl();
            $data = $curl->get($this->ean_base_url . 'info?'.http_build_query($final_request));
            Logger::add($data,'response');
            $code = 200;
        } catch (\Exception $e) {
            $code = 500;
